  ...
  return (
    <nav className="w-full bg-white/80 backdrop-blur border-b border-gray-200 sticky top-0 z-50">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <Link to="/dashboard" className="text-xl font-semibold">Reuse Tourist</Link>
        <div className="flex items-center gap-4">
          ...
          {user ? (
            ...
            <button onClick={handleLogin} className="px-3 py-1.5 rounded bg-indigo-600 text-white hover:bg-indigo-700">Login</button>
          )}
        </div>
      </div>
    </nav>
